...
public abstract class AbstractPrefixSuffix {
  ...
  public PrefixSuffixSaveData save() {
    return new PrefixSuffixSaveData(this);
  }
  ...
  public abstract class FloatEffectLevelData extends NumericEffectLevelData  {
    ...
    public float getRandomFloatValueForLevel(int level) {
      Vector range = getNumericValueRangeForLevel(level);
      Vector intRange = Vector.mulScalar(range, 100).roundAllInst();
      return Util.rand((int)intRange.x, (int)intRange.y)/100f;
    }
  }
  ...
  public static class PrefixSuffixSaveData {
    public Class<? extends AbstractPrefixSuffix> presufClass;
    public int level;
    public EffectsObjSaveData effects;

    public PrefixSuffixSaveData(Class<? extends AbstractPrefixSuffix> newObjClass, int newLevel, EffectsObjSaveData newEffectsSaveData) {
      presufClass = newObjClass;
      level = newLevel;
      effects = newEffectsSaveData;
    }

    public PrefixSuffixSaveData(AbstractPrefixSuffix ps) {
      this(ps.getClass(), ps.level, ps.effects.save());
    }

    public AbstractPrefixSuffix load() {
      AbstractPrefixSuffix result = Util.instantiateClass(presufClass, 0);
      result.effects = new EffectsObj( effects );
      result.level = level;
      return result;
    }
